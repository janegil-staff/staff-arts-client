...
export default function ArtworkDetail() {
  ...
  const price = artwork.price > 0 ? `${artwork.price} ${artwork.currency || 'NOK'}` : 'Not for sale'
      ...
      <div className="px-5 pt-5">
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex-1 min-w-0 pr-3">
            <h1 className="text-2xl font-bold" style={{ fontFamily: 'Playfair Display', color: 'var(--text)' }}>
              {artwork.title}
            </h1>
            <p className="text-xl font-semibold mt-1" style={{ color: 'var(--teal)' }}>
              {price}
            </p>
          </div>
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-full flex-shrink-0"
            style={{ background: artwork.status === 'available' ? 'rgba(38,142,134,0.15)' : 'rgba(255,255,255,0.05)' }}>
            <span className="text-xs font-medium capitalize"
              style={{ color: artwork.status === 'available' ? 'var(--teal)' : 'var(--text-muted)' }}>
              {artwork.status}
            </span>
          </div>
        </div>
        ...
        {/* Artist */}
        {artist && (
          <div className="flex items-center gap-3 p-3 rounded-xl cursor-pointer mb-4 hover:bg-white/5 transition-colors"
            style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border)' }}
            onClick={() => navigate(`/profile/${artistId}`)}>
            <img
              src={artist.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${artistId}`}
              alt={artist.name}
          ...
          <div className="mb-6">
            ...
            <div className="flex gap-2 overflow-x-auto pb-1" style={{ scrollbarWidth: 'none' }}>
              {artwork.images.map((img: any, i: number) => (
                <img key={i} src={img.url} alt={`${artwork.title} ${i + 1}`}
        ...
        {artwork.forSale && artwork.price > 0 && (
          <div className="flex gap-3 mt-2">
            {artist && (
              <button
                onClick={() => navigate(`/messages/new?artistId=${artistId}&artworkId=${artwork._id}`)}
