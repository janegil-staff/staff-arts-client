...
function ShowRow({ item, onClick }: { item: ShowItem; onClick: () => void }) {
  ...
  const meta = [cfg.label, item.subType, item.dateStr, item.location]
    .filter(Boolean)
    .join("  ·  ");
  return (
    <div
...
function NavButton({
  ...
  return (
    <button
      onClick={onClick}
      className="flex-1 flex flex-col items-center py-3 rounded-xl transition-all hover:bg-white/5"
      style={{
    ...
    >
      ...
      {count > 0 && (
        <span
          className="text-xs font-bold mt-1 w-5 h-5 rounded-full flex items-center justify-center"
  ...
  useEffect(() => {
    const load = async () => {
      ...
      for (const e of evtRes) {
        ...
        all.push({
          id: e._id,
          type: e.category === "music" ? "music" : "event",
          title: e.title ?? "",
          subType: (e.type ?? "").replace(/_/g, " "),
          dateStr: d ? formatDate(d) : "",
          location: e.location ?? "",
    ...
    <div className="min-h-screen max-w-3xl mx-auto">
      ...
      >
        <NavButton
          ...
          label="Events"
          count={eventCount}
          onClick={() => navigate("/shows/events")}
        />
        <NavButton
          icon="🖼️"
          label="Exhibitions"
          count={exhibitionCount}
          onClick={() => navigate("/shows/exhibitions")}
        />
        <NavButton
          icon="🎵"
          label="Music"
          count={musicCount}
          onClick={() => navigate("/shows/music")}
        />
      </div>
      ...
      <button
        ...
        style={{
          background: "var(--teal)",
          color: "white",
          boxShadow: "0 4px 20px rgba(38,142,134,0.4)",
        }}
      >
