...
export default function Profile() {
  ...
  const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      await api.patch(`/users/${user._id || user.id}`, { avatar: url })
  ...
  const handleCoverChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const { url } = await uploadImage(file)
      await api.patch(`/users/${user._id || user.id}`, { coverImage: url })
  ...
  return (
    <div className="min-h-screen pb-24 max-w-2xl mx-auto">
      ...
      {/* Avatar */}
      <div className="px-6">
        <div className="relative -mt-11 mb-3" style={{ width: '80px' }}>
          {user.avatar ? (
        ...
        {user.bio && (
          <p className="text-sm mt-3 leading-relaxed" style={{ color: 'var(--text-muted)' }}>{user.bio}</p>
        )}
        {user.location && (
      ...
      <div className="mx-6 mt-4">
        <button onClick={() => navigate(`/edit-profile/${userId}`)}
