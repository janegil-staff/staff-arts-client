...
export default function Upload() {
  ...
  const [form, setForm] = useState({ title: '', description: '', price: '', medium: '', style: '', currency: 'NOK', width: '', height: '', unit: 'cm' })
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await api.post('/artworks', {
        ...
        dimensions: form.width && form.height ? {
  ...
  return (
    <div className="min-h-screen pb-24 px-4 pt-6 max-w-2xl mx-auto">
      ...
      <label className="block mb-6 cursor-pointer mx-auto" style={{ maxWidth: '512px' }}>
        <div className="w-full rounded-2xl flex flex-col items-center justify-center overflow-hidden"
          style={{ border: '2px dashed var(--border)', background: 'var(--bg-elevated)', minHeight: '200px' }}>
          {preview ? (
            ...
            <>
              ...
              <p className="mt-3 text-sm" style={{ color: 'var(--text-muted)' }}>Tap to select image</p>
            </>
          )}
        </div>
      ...
      <div className="space-y-4">
        ...
        <div className="flex gap-2">
          ...
          <input type="number" placeholder="Price — leave 0 if not for sale" value={form.price}
        ...
        <select value={form.medium} onChange={e => setForm(f => ({ ...f, medium: e.target.value }))}
          ...
          {['Oil','Acrylic','Watercolor','Gouache','Tempera','Fresco','Encaustic',
            'Pastel','Charcoal','Pencil','Ink','Graphite','Crayon','Digital',
            'Photography','Printmaking','Lithography','Etching','Screenprint',
            'Sculpture','Ceramic','Glass','Textile','Mixed Media','Other'].map(m => (
        ...
        <select value={form.style} onChange={e => setForm(f => ({ ...f, style: e.target.value }))}
          ...
          {['Abstract','Realism','Impressionism','Expressionism','Surrealism',
            'Minimalism','Pop Art','Cubism','Modernism','Contemporary',
            'Street Art','Folk Art','Illustration','Concept Art','Other'].map(s => (
        ...
        <div>
          <p className="text-xs font-semibold uppercase tracking-wider mb-1.5"
            style={{ color: 'var(--text-muted)' }}>Categories (optional)</p>
          <div className="flex flex-wrap gap-2">
            {['Painting','Drawing','Photography','Sculpture','Digital Art','Printmaking',
              'Illustration','Street Art','Abstract','Portrait','Landscape','Still Life',
              'Wildlife','Architecture','Fashion','Concept Art'].map(cat => (
              <button key={cat} type="button"
                ...
                style={{
                  background: selectedCategories.includes(cat) ? 'var(--teal)' : 'var(--bg-elevated)',
        ...
        <div>
          <p className="text-xs font-semibold uppercase tracking-wider mb-1.5"
            style={{ color: 'var(--text-muted)' }}>Dimensions (optional)</p>
          <div className="flex gap-2">
            <input type="number" placeholder="Width" value={form.width}
            ...
            <select value={form.unit} onChange={e => setForm(f => ({ ...f, unit: e.target.value }))}
              className="px-3 py-3 rounded-xl text-sm outline-none" style={{ ...inputStyle, minWidth: '70px' }}>
              {['cm', 'in', 'mm'].map(u => <option key={u} value={u}>{u}</option>)}
            </select>
          </div>
        </div>
